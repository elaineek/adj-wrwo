...
dsnin 			SAS dataset containing the analysis data. Must be in wide 
				format such that a participantâ€™s repeated responses are in 
...
%MACRO Adj_WinRatio(DSNIN,
	...
	quit;
   	...
   	%if &num_strata. > 0 & &METHOD. = LARGE %then %do;
		...
		%if &min_ss_strata. < 50 %then %do;
      		%put WARNING: Minimum within-stratum sample size is less 50. Consider using METHOD = SMALL instead.;
   		%end;  
	...
	PROC IML;
		...
		/******************************* COMPUTEU_RATIO FUNCTION *********************************/
		start computeU_ratio;
			...
			do i = 1 to ss2;
				do ip = 1 to ss1;
					...
					%if &r_1. > &r. %then %do;
						if (trtData0[i] = . | ctrlData0[ip] = .) then 
							U_iip[counter, loc] = 0;
						else
							U_iip[counter, loc] = (trtData0[i] > ctrlData0[ip]);
						loc = loc + 1;
					%end;

					%do j = 1 %to &num_visits.;

						if (trtData[i, &j.] = . | ctrlData[ip, &j.] = .) then 
							U_iip[counter, loc] = 0;
						else
							U_iip[counter, loc] = (trtData[i, &j.] > ctrlData[ip, &j.]);
						loc = loc + 1;
